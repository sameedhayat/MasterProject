 ...
 * based on the "fixed-likes.ttl" dataset.
 */
public class LikesSrrDriver extends SailRecommenderRepository {
	 ...
	 *        A Sail object.
	 */
	public LikesSrrDriver(Sail sail) {
		super(sail);
	}
	 ...
	 *---------*/      
        ...
        public RatedResource[] getTop2Recommendations(String userURI) throws RecommenderException {
                ...
                if (userURI.equals("http://example.org/fixed#User1")){
                        res1 = new RatedResource("http://example.org/fixed#Item5", 1.0);
                        res2 = new RatedResource("http://example.org/fixed#Item6", 1.0);
                }
                //1/2 matches in two folds. Precision = 0 in one fold
                if (userURI.equals("http://example.org/fixed#User2")){
                        res1 = new RatedResource("http://example.org/fixed#Item4", 1.0);
                        res2 = new RatedResource("http://example.org/fixed#Item1", 1.0);
                }
                //1/2 matches in non adjacent folds. Precision = 0 in one fold
                if (userURI.equals("http://example.org/fixed#User3")){
                        res1 = new RatedResource("http://example.org/fixed#Item1", 1.0);
                        res2 = new RatedResource("http://example.org/fixed#Item5", 1.0);
                }
                //Precision 0
                if (userURI.equals("http://example.org/fixed#User4")){
                        res1 = new RatedResource("http://example.org/fixed#Item5", 1.0);
